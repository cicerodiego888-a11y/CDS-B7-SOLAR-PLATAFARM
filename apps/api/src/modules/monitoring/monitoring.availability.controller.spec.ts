...
describe('Proteção da disponibilidade', () => {
  ...
  it('COMERCIAL não acessa availability (403)', () => {
    reflector.getAllAndOverride.mockImplementation((key: string) => (
      key === PERMISSIONS_KEY ? ['MONITORING_VIEW'] : undefined
    ));
    ...
    expect(guard.canActivate(contextOf({}, { role: 'OPERATOR' }))).toBe(true);
  });
});
